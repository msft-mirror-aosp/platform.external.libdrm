...
# The script is in $AOSP_ROOT/third_party/libdrm/aemu-bazel
AOSP_ROOT=$(realpath "$SCRIPT_DIR/../../..")

# Define absolute paths for config files
CONFIG_FILE="$SCRIPT_DIR/aemu-drm-build-config.jsonc"
SHIM_FILE="$SCRIPT_DIR/aemu-drm-shim.jsonc"

# The directory where the python script should be run from (top of libdrm repo)
